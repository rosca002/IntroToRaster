---
author: "LoÃ¯c Dutrieux, Jan Verbesselt, Johannes Eberenz, Dainius MasiliÅ«nas", Sabina Rosca
...
## Introduction
Raster data, is like any image. Although it may portray various properties of objects in the real world, these objects don’t exist as separate objects; rather, they are represented using pixels of various different color.
...
Today there is no bonus task. Instead, only for this exercise, the 2 points you would normally receive for doing a bonus task, will be given if you have a correct project structure and if you used git properly in this project (exercise 5: "Design a pre-processing chain to assess change in NDVI over time"). The points will be graded in accordance with the table below, and added to the total score for exercise 5 in the Bonus question section of the [Generic Rubric](https://docs.google.com/document/d/1wdqcrqh4Bt-tNwhEFRa4esq7ozwo36tfOjGmbBTK5B4/edit).

 &nbsp; | 0% | 50% | 100%
------------- | ------------- |-------------|-------------
Effective use of git | Repository does not have correct naming, there is no description/very poor description of each push (0p) | Either only correct repository naming or acceptable description for each push (0.25p) | Correct repository naming and acceptable description for each push (0.5p)
Project structure | There is no project structure, all the files are in one folder, and input data was uploaded in the repository instead of having the download as part of the script (0p) | All/some of the folders ("Data", "R", "Output") exist, but files are not structured accordingly (0.25p) | All files are in the correct folder (all folders exist and have files in them after the execution of the script), the file "main.R" being outside the mentioned folders (0.5p)
